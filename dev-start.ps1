...
function Wait-Port {
    param([string]$ComputerName, [int]$Port, [string]$Name, [int]$TimeoutSeconds = 45)
    ...
    Write-Host ""
    Write-Host "$Name did not start listening on ${ComputerName}:${Port} within ${TimeoutSeconds}s." -ForegroundColor Red
    return $false
}
...
# "localhost" (not a hardcoded 127.0.0.1) since uvicorn binds IPv4-only but Vite's dev server binds
# IPv6-only ([::1]) by default on this machine — "localhost" lets .NET try whichever actually answers.
$backendOk = Wait-Port -ComputerName "localhost" -Port 8000 -Name "Backend"
$frontendOk = Wait-Port -ComputerName "localhost" -Port 5173 -Name "Frontend"
